...
class HostsManager {
    ...
    verifyHostsSection(expectedDomains) {
        ...
        const sectionPresent =
            text.includes(this.blockerMarkerStart) &&
            text.includes(this.blockerMarkerEnd);

        const found = sectionPresent ? this.extractBlockedDomainsFromHostsText(text) : [];
        ...
        // When blocking nothing, markers should ideally be absent; allow markers with zero lines as non-ok-extra handled above.
        if (expSet.size > 0) {
            ok = ok && sectionPresent;
        } else {
            ok = ok && !sectionPresent;
        }
